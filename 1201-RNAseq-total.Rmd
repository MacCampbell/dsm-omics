...
pheatmap(assay(ntd[rownames(ntd) %in% results05fc1$Gene,]), cluster_rows=FALSE, show_rownames=FALSE,
         cluster_cols=TRUE, clustering_method = "ward.D2", annotation_col=df, show_colnames = FALSE)
...
```{r}
pcaData <- plotPCA(vsd[rownames(vsd) %in% results10$Gene,], intgroup=c("Type"), returnData=TRUE) %>% 
  left_join(sum, by=c("name"="Sample"))
percentVar <- round(100 * attr(pcaData, "percentVar"))
...
```{r}
ggplot(tdf) + geom_boxplot(aes(x=Sample, y=Count)) +
  theme_bw() +
