...
```{r}
m80 %>% select(`Sample name`) %>% write_tsv("meta/m80-sample-names.txt")
females<-m80 %>% filter(Pheno==1)
females %>% select(`Sample name`) %>% write_tsv("meta/females.txt")
males<-m80 %>% filter(Pheno==0)
males %>% select(`Sample name`) %>% write_tsv("meta/males.txt")
```
...
#renaming vcf
bcftools reheader --samples bamlists/m80-sample-names.txt -o outputs/1300/renamed.vcf.gz outputs/1300/plink.vcf.gz
```
